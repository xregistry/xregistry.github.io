...
from typing import Iterable, List, NewType, Optional, Sequence, Set, Tuple, TypeVar, Union
...
async def _directory_issues(
    source: Union[Path, Path], settings: Settings
) -> Iterable[TaggedIssue]:
    # Determine file paths based on input
    file_paths = (
        sorted(_all_docs(source, settings.excluded_paths))
        if source.is_dir()
        else [source]
    )
